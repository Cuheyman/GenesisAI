...
MOMENTUM_TAKE_PROFIT = [
    {"minutes": 15, "profit_pct": 2.0},   # 2% in 15 minutes
    {"minutes": 30, "profit_pct": 1.5},   # 1.5% in 30 minutes
    {"minutes": 60, "profit_pct": 1.0},   # 1% in 1 hour
    {"minutes": 120, "profit_pct": 0.5},  # 0.5% in 2 hours
]

# Regular trade profit targets
REGULAR_TAKE_PROFIT = [
    {"minutes": 60, "profit_pct": 3.0},   # 3% in 1 hour
    {"minutes": 120, "profit_pct": 2.0},  # 2% in 2 hours
    {"minutes": 240, "profit_pct": 1.5},  # 1.5% in 4 hours
    {"minutes": 360, "profit_pct": 1.0},  # 1% in 6 hours
]
...
# Stop loss settings
MOMENTUM_STOP_LOSS = float(os.getenv('MOMENTUM_STOP_LOSS', '-1.5'))  # 1.5% stop loss for momentum trades
QUICK_STOP_LOSS = float(os.getenv('QUICK_STOP_LOSS', '-2.0'))     # 2% for regular quick trades
...
EXPANDED_CRYPTO_LIST = [
    ...
    'FIL', 'ARB', 'FET', 'ATOM', 'THETA', 'BONK', 'EOS', 'XTZ', 'IOTA',
    'NEO', 'EGLD', 'ZEC', 'LAYER',
...
OPPORTUNITY_SCAN_INTERVAL = int(os.getenv('OPPORTUNITY_SCAN_INTERVAL', '45'))  # Scan every 45 seconds
...
QUICK_SCAN_INTERVAL = int(os.getenv('QUICK_SCAN_INTERVAL', '15'))  # Ultra-fast scans every 15 seconds
...
def validate_config():
    ...
    if OPPORTUNITY_SCAN_INTERVAL < 30:
